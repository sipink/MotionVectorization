...
class CoTracker3TrackerEngine:
    ...
    def _initialize_engine(self):
        ...
        print(
            f"   Grid size: {self.config.grid_size}×{self.config.grid_size} = {self.config.grid_size**2} points"
        )
        print(
            f"📊 CoTracker3 available: {COTRACKER3_AVAILABLE}, CUDA available: {torch.cuda.is_available()}"
        )
    ...
    def _validate_installation(self):
        ...
        try:
            ...
            if self.config.mixed_precision and self.device.type == "cuda":
                test_video = torch.randn(
                    *test_shape, dtype=torch.float16, device=self.device
                )
            else:
                test_video = torch.randn(
                    *test_shape, dtype=torch.float32, device=self.device
                )
            ...
            with torch.no_grad():
                try:
                    tracks, visibility = self.track_video_grid(
                        test_video, grid_size=test_grid_size
                    )
                    expected_points = test_grid_size * test_grid_size

                    if (
                        tracks.shape[2] == expected_points
                        and visibility.shape[2] == expected_points
                    ):
                        print("✅ Grid tracking validation passed")
                    else:
                        print(
                            f"⚠️ Grid tracking shape mismatch: got {tracks.shape[2]}, expected {expected_points}"
                        )
    ...
    def _track_custom_points(
        self, video: torch.Tensor, custom_points: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def _track_grid_points(
        self, video: torch.Tensor, grid_size: int, H: int, W: int
    ) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def _apply_temporal_smoothing(self, tracks: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(1, tracks.shape[1]):
            ...
            valid_prev = ~torch.isnan(smoothed_tracks[:, t - 1]).any(
                dim=-1, keepdim=True
            )
    ...
    def _track_video_fallback(
        ...
        tracks_tensor = torch.tensor(
            np.stack(all_tracks), dtype=torch.float32, device=self.device
        )
        visibility_tensor = torch.tensor(
            np.stack(all_visibility), dtype=torch.float32, device=self.device
        )
    ...
    def _track_sequence_opencv(
        ...
        for t in range(1, T):
            ...
            valid = (
                (status.ravel() == 1) & (back_status.ravel() == 1) & (distance < 1.0)
            )
    ...
    def _extract_contour_points(
        self, masks: torch.Tensor, obj_id: int, sample_density: int
            ...
            if obj_masks[0, t].any():
                ...
                if contours:
                    ...
                    if len(largest_contour) >= sample_density:
                        # Uniform sampling
                        indices = np.linspace(
                            0, len(largest_contour) - 1, sample_density, dtype=int
                        )
    ...
    def extract_motion_parameters(
        ...
        motion_params = {
            ...
            "affine_matrix": torch.eye(3, device=self.device)
            .unsqueeze(0)
            .unsqueeze(0)
            .repeat(B, T, 1, 1),
        }
            ...
            for t in range(1, T):
                ...
                try:
                    ...
                    if len(points_prev) >= 3:
                        ...
                        # Store parameters
                        motion_params["translation"][b, t] = torch.tensor(
                            translation, device=self.device
                        )
                        motion_params["scale"][b, t] = torch.tensor(
                            scale, device=self.device
                        )
                        motion_params["rotation"][b, t] = torch.tensor(
                            rotation, device=self.device
                        )

                        # Store full affine matrix
                        M_full = np.eye(3)
                        M_full[:2, :] = M
                        motion_params["affine_matrix"][b, t] = torch.tensor(
                            M_full, device=self.device
                        )
    ...
    def _calculate_performance_rating(self) -> str:
        ...
        accuracy = (
            np.mean(self.performance_stats["accuracy_scores"])
            if self.performance_stats["accuracy_scores"]
            else 0.0
        )
        ...
        if fps >= self.config.target_fps and accuracy >= self.config.accuracy_target:
            ...
            fps >= self.config.target_fps * 0.8
            and accuracy >= self.config.accuracy_target * 0.9
        ):
            return "GOOD"
        elif (
            fps >= self.config.target_fps * 0.6
            and accuracy >= self.config.accuracy_target * 0.8
        ):
            return "FAIR"
        else:
...
def create_cotracker3_engine(
    mode: str = "offline", device: str = "auto", grid_size: int = 50, **kwargs
    ...
    config = CoTracker3Config(
        model_variant=f"cotracker3_{mode}", device=device, grid_size=grid_size, **kwargs
    )
