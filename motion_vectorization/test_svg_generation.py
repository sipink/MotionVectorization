    ...
    def test_svg_generation_basic(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success = create_svg_from_motion_file(
                str(motion_file_path), str(shapes_dir), str(svg_output_path), config
            )
            ...
            if success and svg_output_path.exists():
                ...
                result["details"]["contains_animations"] = (
                    "animateTransform" in svg_content
                )
    ...
    def test_motion_file_format(self) -> Dict[str, Any]:
        ...
        try:
            ...
            checks = {
                ...
                "metadata_has_width": motion_data.get("-1", {}).get("width")
                is not None,
                "metadata_has_height": motion_data.get("-1", {}).get("height")
                is not None,
                "has_shapes": len([k for k in motion_data.keys() if k != "-1"]) > 0,
                "shapes_have_time": all(
                    "time" in motion_data[k] for k in motion_data if k != "-1"
                ),
                "shapes_have_transforms": all(
                    all(
                        param in motion_data[k]
                        for param in ["cx", "cy", "sx", "sy", "theta", "kx", "ky"]
                    )
    ...
    def test_unified_pipeline_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_path in expected_files:
                ...
                if file_exists:
                    result["details"][
                        f"{file_path.name}_size"
                    ] = file_path.stat().st_size

            # Determine success
            result["success"] = all(
                result["details"].get(f"{f.name}_exists", False) for f in expected_files
            )
