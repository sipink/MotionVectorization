...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--output_dir', default='motion_vectorization/outputs', type=str, 
    ...
    parser.add_argument('--use_unified_pipeline', action='store_true', default=True,
                       help='Use unified SAM2.1 + CoTracker3 + FlowSeek pipeline')
    parser.add_argument('--unified_mode', type=str, default='balanced', 
                       choices=['speed', 'balanced', 'accuracy'],
                       help='Processing mode: speed (60 FPS), balanced (44 FPS), accuracy (30 FPS)')
    parser.add_argument('--unified_device', type=str, default='auto', 
                       choices=['auto', 'cuda', 'cpu'],
                       help='Processing device')
    parser.add_argument('--progressive_fallback', action='store_true', default=True,
                       help='Enable progressive fallback to individual engines')
    parser.add_argument('--quality_threshold', type=float, default=0.90,
                       help='Minimum quality threshold')
    parser.add_argument('--benchmark_performance', action='store_true', default=False,
    ...
    parser.add_argument('--use_cotracker3', action='store_true', default=True,
                       help='Use CoTracker3 for tracking')
    parser.add_argument('--cotracker3_mode', type=str, default='offline', 
    ...
    parser.add_argument('--min_cluster_size', default=100, type=int, 
                       help='Minimum cluster size for shape detection')
    parser.add_argument('--min_density', default=0.20, type=float, 
                       help='Minimum cluster density')
...
def setup_output_directories(video_name: str, args) -> Dict[str, str]:
    ...
    output_base = os.path.join(args.output_dir, f'{video_name}_{args.suffix or "unified"}')
...
def process_with_unified_pipeline(
    ...
    try:
        ...
        if args.progressive_fallback:
            print("🔄 Progressive fallback enabled - would continue with individual engines")
...
def setup_individual_engines_fallback(args) -> Dict[str, Any]:
    ...
    if args.use_flowseek:
        try:
            ...
            print(f"✅ FlowSeek fallback engine ready (depth: {args.flowseek_depth_integration})")
        except Exception as e:
...
def main():
    ...
    for arg_name, arg_val in vars(args).items():
        if arg_name in ['use_unified_pipeline', 'unified_mode', 'use_sam2', 'use_cotracker3', 'use_flowseek']:
    ...
    if args.use_unified_pipeline:
        try:
            results = process_with_unified_pipeline(video_name, video_path, directories, args)
            
            if results.get('status') == 'fallback_required' and args.progressive_fallback:
