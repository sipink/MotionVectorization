...
def check_engine_availability() -> Dict[str, Dict]:
        ...
        try:
            if torch.cuda.is_available():
                ...
                try:
                    ...
                    status["gpu"]["cuda_version"] = "Unknown"
                status["gpu"]["details"] = [
                    f'✅ GPU detected: {status["gpu"]["device"]} with {status["gpu"]["memory"]} memory'
                ]
            else:
    ...
    try:
        ...
        if models_found:
            status["sam2"]["details"].append(
                f'✅ Found models: {", ".join(models_found)}'
            )
        else:
        ...
        if TORCH_AVAILABLE and torch is not None:
            try:
                ...
                if hasattr(torch, "hub"):
                    ...
                    status["cotracker3"]["details"].append(
                        "✅ torch.hub available for CoTracker3"
                    )
                    ...
                    try:
                        ...
                        torch.hub.list(
                            "facebookresearch/co-tracker", force_reload=False
                        )
                        status["cotracker3"]["details"].append(
                            "✅ CoTracker3 models accessible"
                        )
                    except Exception as hub_e:
                        status["cotracker3"]["details"].append(
                            f"⚠️ CoTracker3 hub access: {hub_e}"
                        )
                else:
                    status["cotracker3"]["details"].append("❌ torch.hub not available")
            except Exception as e:
                status["cotracker3"]["details"].append(
                    f"❌ CoTracker3 import error: {e}"
                )
        else:
            status["cotracker3"]["details"].append(
                "❌ PyTorch not installed - CoTracker3 unavailable"
            )
    ...
    try:
        ...
        if TORCH_AVAILABLE and torch is not None:
            try:
                if hasattr(torch, "hub"):
                    ...
                    status["flowseek"]["details"].append(
                        "✅ MiDaS depth models available via torch.hub"
                    )
                    depth_available = True
            except Exception:
                pass
        ...
        if depth_available:
            ...
            status["flowseek"]["details"].append(
                "❌ No depth models available for FlowSeek"
            )
    ...
    try:
        from .unified_pipeline import (
            UnifiedMotionVectorizationPipeline,
        ...
        if engines_active:
            status["unified_pipeline"]["details"].append(
                f'✅ Active engines: {", ".join(engines_active)}'
            )
        else:
            status["unified_pipeline"]["details"].append(
                "⚠️ No AI engines active - will use fallback methods"
            )

    except Exception as e:
        status["unified_pipeline"]["details"].append(
            f"❌ Unified pipeline error: {str(e)}"
        )
...
def print_engine_status(status: Dict[str, Dict], verbose: bool = True):
    ...
    print(
        f"{Colors.BOLD}{Colors.HEADER}🚀 MOTION VECTORIZATION ENGINE STATUS REPORT{Colors.ENDC}"
    )
    print(f"{Colors.BOLD}{Colors.HEADER}{'='*80}{Colors.ENDC}")
    print(
        f"{Colors.OKCYAN}Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{Colors.ENDC}"
    )
    print(
        f"{Colors.OKCYAN}PyTorch: {status['system']['pytorch_version']} | Python: {status['system']['python_version']}{Colors.ENDC}"
    )
    ...
    if status["sam2"]["available"]:
        print(
            f"  {Colors.OKGREEN}✅ AVAILABLE - {status['sam2']['version']}{Colors.ENDC}"
        )
    else:
    ...
    if status["cotracker3"]["available"]:
        print(
            f"  {Colors.OKGREEN}✅ AVAILABLE - {status['cotracker3']['version']}{Colors.ENDC}"
        )
    else:
        print(f"  {Colors.FAIL}❌ NOT AVAILABLE{Colors.ENDC}")
    ...
    if status["flowseek"]["available"]:
        print(
            f"  {Colors.OKGREEN}✅ AVAILABLE - {status['flowseek']['version']}{Colors.ENDC}"
        )
    else:
        print(f"  {Colors.FAIL}❌ NOT AVAILABLE{Colors.ENDC}")
    ...
    if status["unified_pipeline"]["available"]:
        print(
            f"  {Colors.OKGREEN}✅ AVAILABLE - {status['unified_pipeline']['version']}{Colors.ENDC}"
        )
    else:
        print(f"  {Colors.FAIL}❌ NOT AVAILABLE{Colors.ENDC}")
    ...
    if active_engines == 3:
        print(
            f"  {Colors.OKGREEN}✅ ALL ENGINES OPERATIONAL - MAXIMUM ACCURACY MODE{Colors.ENDC}"
        )
        print(
            f"  {Colors.OKGREEN}   Expected accuracy: 95-100% (World-class){Colors.ENDC}"
        )
    elif active_engines >= 2:
        print(
            f"  {Colors.WARNING}⚠️  {active_engines}/3 ENGINES OPERATIONAL - GOOD ACCURACY MODE{Colors.ENDC}"
        )
        print(f"  {Colors.WARNING}   Expected accuracy: 85-95%{Colors.ENDC}")
    elif active_engines == 1:
        print(
            f"  {Colors.WARNING}⚠️  ONLY 1 ENGINE OPERATIONAL - LIMITED ACCURACY MODE{Colors.ENDC}"
        )
        print(f"  {Colors.WARNING}   Expected accuracy: 70-85%{Colors.ENDC}")
    else:
        print(
            f"  {Colors.FAIL}❌ NO AI ENGINES OPERATIONAL - FALLBACK MODE{Colors.ENDC}"
        )
        print(
            f"  {Colors.FAIL}   Expected accuracy: 50-70% (Legacy methods only){Colors.ENDC}"
        )
    ...
    print(
        f"    • GPU acceleration: {'ENABLED' if status['gpu']['available'] else 'DISABLED (CPU mode)'}"
    )
    ...
    if active_engines < 3:
        ...
        if not status["sam2"]["available"]:
            print(
                f"  {Colors.WARNING}• Install SAM2: pip install segment-anything-2{Colors.ENDC}"
            )
            print(
                f"  {Colors.WARNING}• Download models: Place sam2_hiera_large.pt in models/{Colors.ENDC}"
            )
        if not status["cotracker3"]["available"]:
            print(
                f"  {Colors.WARNING}• Ensure torch.hub access for CoTracker3{Colors.ENDC}"
            )
        if not status["flowseek"]["available"]:
            print(
                f"  {Colors.WARNING}• Install depth models: pip install transformers{Colors.ENDC}"
            )
...
def ensure_maximum_accuracy() -> bool:
    ...
    if len(active_engines) < 3:
        print(
            f"{Colors.WARNING}⚠️  WARNING: Not all AI engines are active!{Colors.ENDC}"
        )
        print(
            f"{Colors.WARNING}   Active engines: {', '.join(active_engines) if active_engines else 'NONE'}{Colors.ENDC}"
        )
        print(
            f"{Colors.WARNING}   Missing engines: {', '.join(set(['sam2', 'cotracker3', 'flowseek']) - set(active_engines))}{Colors.ENDC}"
        )
        return False
