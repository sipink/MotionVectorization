...
torch = None  # Initialize torch as None to avoid UnboundLocalError
...
def check_engine_availability() -> Dict[str, Dict]:
    ...
    """
    # Get PyTorch version safely
    if TORCH_AVAILABLE and torch is not None:
        try:
            pytorch_version = torch.__version__
        except:
            pytorch_version = 'Unknown version'
    else:
    ...
    if TORCH_AVAILABLE and torch is not None and torch.cuda.is_available():
        ...
        try:
            ...
            status['gpu']['details'] = [f'GPU detected but details unavailable: {e}']
    else:
        if not TORCH_AVAILABLE:
            status['gpu']['details'] = ['PyTorch not installed - cannot detect GPU']
        else:
            status['gpu']['details'] = ['No CUDA GPU detected - will use CPU (slower performance)']
...
def print_engine_status(status: Dict[str, Dict], verbose: bool = True):
    ...
    if status['gpu']['available']:
        ...
        print(f"     Device: {status['gpu']['device']}")
        print(f"     Memory: {status['gpu']['memory']}")
        print(f"     CUDA Version: {status['gpu']['cuda_version']}")
    else:
