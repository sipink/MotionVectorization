...
parser.add_argument(
    "--video_name", required=True, type=str, help="Name of video to process."
)
parser.add_argument(
    "--video_dir", default="videos", type=str, help="Directory containing video data."
)
parser.add_argument(
    "--output_dir", default="outputs", type=str, help="Directory to save outputs."
)
parser.add_argument(
    "--suffix", default=None, type=str, help="Suffix for output video names."
)
parser.add_argument("--bg_file", type=str, default=None, help="Background file.")
parser.add_argument(
    "--skip_figs", action="store_true", default=False, help="Skip generating figures."
)
...
def main():
    ...
    shapes_folder = os.path.join(
        arg.output_dir, f"{arg.video_name}_{arg.suffix}", "shapes"
    )
    ...
    for filename in os.listdir(shapes_folder):
        if os.path.splitext(filename)[1] == ".png":
            ...
            shape = cv2.imread(
                os.path.join(shapes_folder, filename), cv2.IMREAD_UNCHANGED
            )
            shapes[shape_idx] = shape
    print("\nShapes:", shapes.keys())

    # shape_bank_name = 'shape_bank.pkl' if arg.suffix is None else f'shape_bank_{arg.suffix}.pkl'
    shape_bank = pickle.load(
        open(
            os.path.join(
                arg.output_dir, f"{arg.video_name}_{arg.suffix}", "shape_bank.pkl"
            ),
            "rb",
        )
    )
    ...
    for i, shape_idx in tqdm(enumerate(shape_bank)):
        ...
        shape_data = {
            ...
            "time": [
                int(frame_idxs[shape_idx_info["t"]]) for shape_idx_info in shape_info
            ],
            "sx": [shape_idx_info["h"][0] for shape_idx_info in shape_info],
            "sy": [shape_idx_info["h"][1] for shape_idx_info in shape_info],
            "cx": [
                shape_idx_info["centroid"][0] for shape_idx_info in shape_info
            ],  # Keep in pixels for 100% accuracy
            "cy": [
                shape_idx_info["centroid"][1] for shape_idx_info in shape_info
            ],  # Keep in pixels for 100% accuracy
    ...
    with open(
        os.path.join(
            arg.output_dir, f"{arg.video_name}_{arg.suffix}", "motion_file.json"
        ),
        "w",
        encoding="utf-8",
    ) as handle:
